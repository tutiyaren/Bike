...
@section('css')
<link rel="stylesheet" href="{{ asset('css/favorite/scenery/create.css') }}">
@endsection
...
<div class="favoriteCreate-main">
    ...
    <form action="{{ route('scenery.store') }}" method="post" class="favoriteForm" enctype="multipart/form-data">
        @csrf
        <!-- 投稿の選択 -->
        <div class="select">
            ...
            <select name="scenery_genre" id="scenery_genre" class="select-genre">
                <option selected disabled>全てのジャンル</option>
                @foreach($sceneryGenres as $sceneryGenre)
                <option value="{{ $sceneryGenre->id }}" {{ old('scenery_genre') == $sceneryGenre->id ? 'selected' : '' }}>{{ $sceneryGenre->genre }}</option>
                @endforeach
            </select>
        </div>
        @foreach (['area', 'scenery_genre'] as $field)
        @error($field)
